...
> A remote service has many clients accessing a function it provides. The service is a legacy application and is impossible to update. 
Large numbers of requests from users are causing connectivity issues. 
New rules for request frequency should be implemented along with latency checks and client-side logging.
远程服务有许多客户端访问它提供的功能。该服务是一个遗留应用程序，不可能更新。
来自用户的大量请求正在导致连接问题。
请求频率的新规则应该与延迟检查和客户端日志记录一起实现。
